...
`/preview` 目前支援點選任一照片後在上方 Before/After 載入該照片，並以同步 preview API 顯示手動調整結果。照片卡片的版本下拉會切換卡片圖片、上方 Before/After 基準圖、後續手動調整來源與單張下載目標。可對每張照片獨立點按向左/向右 90 度旋轉，並可調整曝光、對比、亮部、暗部、色溫、色偏、飽和、自然飽和、清晰度、銳利化與 HSL 六色區。水平旋轉、裁切縮放/偏移與手動變形修正集中在全螢幕單圖「構圖 / 幾何調整」工作區，使用者可用格線覆蓋比對水平、拖曳裁切框，並用取消/完成控制是否套用。每次滑桿/旋轉只會自動儲存該照片草稿；只有按「產生目前版本」或「產生已選版本」才會建立可下載的 `manual-vN` 版本，並更新 `processed_paths.adjusted` 指向最新版本。匯出 zip 時優先使用最新 `adjusted`，再 fallback 到 pipeline preset output，最後才用原圖。
