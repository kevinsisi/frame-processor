...
- `showroom_white` 需內建足夠對比，不要依賴使用者每張再手動加 contrast；對比提升應優先做 luma-only，避免 RGB contrast 放大 chroma grid 或改變車色。大面積白/灰車身平滑漸層若因 8-bit 色調運算、降噪移除自然顆粒或 JPEG preview 壓縮產生 posterization，主要靠 `_showroom_white` 鏈在 float32 連續空間執行（避免中途 uint8 round-trip 累積階差）+ 最後一次量化前的 luma dither（`SHOWROOM_WHITE_DITHER_STRENGTH = 1.5`，約 ±0.003 luma，破均勻漸層的量化階差）+ 高品質 JPEG 緩解，而不是加強銳化或 chroma noise。`SHOWROOM_WHITE_DITHER_STRENGTH` 必須保持 > 0；停用 dither 會讓後段 cap 函數製造的空間不均勻性直接印到輸出上，外觀像髒污（v0.5.18 教訓）。
- `_limit_smooth_neutral_luma_lift` 的 smooth 區域判斷（`effective_detail`）只能使用 luma HF（source_detail / current_detail），禁止加入 Sobel gradient 項。gradient 判斷會把車身線條、徽章周邊的結構邊緣當成「非 smooth」，造成這些邊緣附近的 luma cap 深度與相鄰漆面不一致，在白色漆面上產生陰影斑塊（「髒污」外觀）。v0.5.11~v0.5.19 共 9 次 fix 均源自此架構問題，v0.5.20 移除 gradient 項後解決（見 memory: feedback-showroom-gradient-cap-dirt）。
