...
- 取代 placeholder dev deploy workflow，改為 Tailscale SSH 到 Windows desktop `100.83.112.20`，複製 `deploy/docker-compose.yml`，執行 `docker compose pull && docker compose up -d`，再跑 health check。
