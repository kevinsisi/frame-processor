...
- 不做 manual version archive / restore UI（clear flow 採 hard delete，不保留手動版本歷史）
...
### Decision 2: 「清空照片微調」是 hard delete

**選項**：
- A. Archive `photo_adjustment_versions`（檔案保留、`archived_at` 寫入、版本下拉跳過）。
- B. 真的刪除磁碟上 `manual-v<N>.jpg` + DB row（含 `photo_adjustment` draft）。
- C. 兩種都提供（按鈕：archive；modal：永久刪除）。

**決定**：B（hard delete）。

**理由**：實作時發現 `photo_adjustment_versions` 沒有 `archived_at` 欄位（與 `processing_jobs` 不對稱），原本設計假設可 archive 是錯誤的。比起加 schema 違反「DB 不動」原則，hard delete 更符合使用者語意：使用者最早就講「preset 就是快速設定，移除了就應該不影響」，「清空」對應的是完整清除，不是可回滾的封存。AI 版本 archive 的存在是因為 GPU 跑一次成本高需要保留 history；手動版本 sliders 拉幾秒就能重做，不需要保留歷史檔案。Decision 從 A 翻成 B 的同時，design §8 的「archive vs delete」開放問題也直接消失（語意確定為 delete）。
...
- **[「清空」是 hard delete，使用者無法回滾]** → 按鈕旁 hint 文明確寫「會刪除已產生的手動版本檔案」；考慮加 `window.confirm` 對「清空已選 N 張」做二次確認（清空目前單張不加，因為操作頻繁）。
...
- **[清空 endpoint 失敗中途，部分照片已 reset 部分沒]** → endpoint 對每張照片使用 DB transaction 包住 reset + DELETE 兩步、再刪檔案；DB commit 後才實際刪檔，磁碟刪檔失敗用 best-effort + log，不阻斷後續照片。每張照片是獨立的 unit。
- **[整合測試 deferred 到 `add-frame-test-harness`]** → 第一版優先 pure-function 服務層測試（clear plan 與 post-commit best-effort 刪檔邏輯），TestClient 端對端測試先列 deferred。風險：服務層邏輯與 router 串接的 bug 沒被 CI 抓到，靠手動 smoke 收尾。
- **[使用者點「清空」後想救回手動 vN]** → 不支援，這是 hard delete 的代價。FE「清空已選」加 `window.confirm` 二次確認降低誤觸。AI 版本仍有 archive 機制保留可回滾，跟手動版本性質不同。
...
- 「清空已選 N 張」是否該擋一次 `window.confirm`？傾向加，避免不可回滾誤觸。
