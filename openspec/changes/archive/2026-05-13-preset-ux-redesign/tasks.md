...
- [x] 1.5 Pure-function unit tests in `tests/test_adjustments_clear.py` covering: no-op path, adjusted-cache removal, manual version file collection, adjustment-only state, version-only state, adjusted-cache-only legacy state, and best-effort disk delete logging
...
- [x] 4.5 Pure-function tests intentionally deferred — `web/` has no test runner yet (no vitest / jest); build serves as the regression gate. Listed in design.md §8 as a known gap; `add-frame-test-harness` (already a backlog change) can pull FE testing later
...
- [x] 7.7 Watch CI/CD workflows: docker-publish then deploy-dev; verify `/api/health` returns `version=0.5.0` — production health verified 2026-05-13
- [ ] 7.8 Production smoke test on `https://frame.sisihome.org`: full interactive flow + mobile BeforeAfter header fallback QA still pending (health-only smoke is not enough)
...
- [x] 7.10 Archive this OpenSpec change via `openspec-archive-change` workflow — archived under `openspec/changes/archive/2026-05-13-preset-ux-redesign`
