...
- **FE 新「清空照片微調」動作**：兩顆按鈕（清空目前 / 清空已選），跟「刪除 preset」徹底分開；reset sliders + hard-delete `photo_adjustment_versions` + 切回 AI vN 或原圖。
- **BE 新 endpoint**：`POST /projects/{id}/adjustments/clear`，接 `photo_ids[]`，原子化執行 reset + hard-delete + recompute active version。
