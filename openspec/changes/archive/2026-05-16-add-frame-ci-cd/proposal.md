...
- 新增正式 Docker publish workflow，push `main` 時建置並發布 CPU API image、CUDA worker image 與 web image。
- 取代 placeholder dev deploy workflow，改為在 kevinhome GitHub self-hosted runner 本機部署 Windows desktop `100.83.112.20`，複製 `deploy/docker-compose.yml`，用臨時 Docker auth config 逐一 pull images 後執行 `docker compose up -d --pull never --force-recreate --no-build --remove-orphans postgres redis api worker web`，再跑 health check。
- 調整 production compose 使用 registry image 與 `${IMAGE_TAG}`，讓 CD 固定部署已發布的 commit SHA images。
...
- GitHub Actions secrets and variables: Docker Hub credentials, Gemini fallback key, settings admin token, optional key-manager URL
